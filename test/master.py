...
class MasterNode:
    def __init__(self, host='0.0.0.0', port=9999):
        ...
        self.worker_queues = {}  # Worker Node의 큐 상태 저장
    ...
    def handle_worker(self, client_socket, address):
        ...
        self.worker_queues[worker_id] = 0  # Worker의 초기 작업 큐 크기를 0으로 설정
        ...
        while True:
            ...
            if not self.failed_queue.empty():
                ...
                for worker_socket in self.worker_sockets:
                    ...
                    if self.worker_queues[worker_id] < 10:  # 큐가 가득 차지 않은 worker에게만 분배
                        worker_socket.send(failed_task_data.encode('utf-8'))
                        print(f"Resent failed task to {worker_id}")
                        self.worker_queues[worker_id] += 1  # 큐에 작업 추가
            ...
            while True:
                ...
                if result:
                    if "failed" in result:
                        ...
                        self.worker_queues[self.worker_ids[worker_socket]] -= 1  # 작업 실패 시 큐에서 제거
                    else:
                        print(f"Received result from {self.worker_ids[worker_socket]}: {result}")
                        self.worker_queues[self.worker_ids[worker_socket]] -= 1  # 작업 성공 시 큐에서 제거
